...
import { Save, Smile, MessageSquare, History, Loader2 } from "lucide-react";
...
export default function ReflectPage() {
  ...
  const handleSave = async () => {
    if (!content.trim() || !user?.uid || !db || isSaving) {
      if (!user?.uid) {
        toast({ variant: "destructive", title: "Authentication Error", description: "You must be logged in to save memories." });
      }
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 pb-10 md:pb-20">
      <header className="space-y-1 md:space-y-2 text-white">
        ...
        <TabsContent value="entry" className="space-y-4 md:space-y-6">
          <div className="relative group">
            ...
            <Textarea
              ...
              disabled={isSaving}
              className="min-h-[300px] md:min-h-[400px] text-lg md:text-xl font-light leading-relaxed bg-card/40 glass-morphism border-white/10 p-5 md:p-8 rounded-2xl focus:ring-primary/40 focus:border-primary/40 transition-all placeholder:text-muted-foreground/30 text-white"
            />
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
            ...
            <Button 
              size="lg" 
              onClick={handleSave}
              disabled={!content.trim() || !user?.uid || isSaving}
              className="w-full md:w-auto px-10 h-12 md:h-14 rounded-full font-headline tracking-widest text-base md:text-lg group overflow-hidden relative shadow-lg shadow-primary/20"
            >
              <span className="relative z-10 flex items-center gap-2">
                {isSaving ? "Syncing..." : "Archive Memory"} 
                {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4 md:w-5 md:h-5" />}
              </span>
            ...
            >
              <div className="space-y-2">
                <div className="flex justify-between items-start">
                  <h3 className="font-headline text-lg md:text-xl font-medium text-white">{session.title}</h3>
              ...
              <Button variant="outline" className="w-fit h-9 md:h-10 rounded-full text-xs md:text-sm border-white/10 text-white hover:bg-primary hover:text-primary-foreground hover:border-transparent transition-all">
                Begin Session
              </Button>
            </motion.div>
          ))}
